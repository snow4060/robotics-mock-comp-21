...
void calibrate_gyro(int readings)
{
    int i = 0;
    double avg = 0;
    while(i < readings)
...
void cg(int readings)
{
    calibrate_gyro(readings);
}
...
void turn_with_gyro(int left_wheel_speed, int right_wheel_speed, double targetTheta)
{
    ...
    while(theta < targetTheta*(580000/90))
    {
...
{ 
    ...
    while(seconds() - startTime < (time / 1000.0))
    {
        if(speed > 0)
        {
            mav(right_motor, (double)(speed - speed * (1.9201 + 0.00*theta - 7.399285e-28*pow(theta, 2) - 2.054177e-18*pow(theta, 3) + 1.3145e-40*pow(theta, 4))));  //here at NAR, we are VERY precise
